...
public class SearchGroupsGrid extends GroupsGrid {
	...
	public SearchGroupsGrid(final SearchCriteriaForGroupsPanel searchCriteriaForGroupsPanel) {
		...
		loader = new BasePagingLoader<PagingLoadResult<ModelData>>(  
			...
			protected void loadData(final Object config) {
				...
				SearchForBlogsContainer.getInstance().resize(0,0);
			}
		}; 
	...
	public void load(Object loadConfig,
		...
		if (errors.size() > 0) {
			...
			errorPanel.show();	
			SearchForBlogsContainer.getInstance().getPagingToolBar().setEnabled(true);
			SearchForBlogsContainer.getInstance().getPagingToolBar().setImages(SearchForBlogsContainer.getInstance().getPagingToolBar().getImages());
